...
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
    <script type="text/javascript" src="resources/js/pc/page.js"></script>
	...
	<div class="paging">
		<input type="button" class="dib" onclick="getPageList('index');return false;">首页</input>
		<input type="button" class="dib" onclick="getPageList('upage');return false;">上一页</input>
        ...
        <c:choose>
	        <c:when test="${requestScope.page.pageCount <= 10}">
	         	...
	         	<c:forEach begin="1" end="${requestScope.page.pageCount }" step="1" var="p" varStatus="status">
	         		<a href="javascript:;" onclick="gotoSelectPage(${p});return false;" <c:if test="${requestScope.page.currentPage == p}"> class="current"</c:if>>${p }</a>
		         </c:forEach>
        ...
        <c:if test="${requestScope.page.pageCount - requestScope.page.currentPage > 5}">
			<span>...</span>
			<a href="javascript:;">${requestScope.page.pageCount}</a>
        </c:if>
		<input type="button" class="dib" onclick="getPageList('next');return false;">下一页</input>
		<input type="button" class="dib" onclick="getPageList('last');return false;">尾页</input>
		<span>&nbsp;&nbsp;到第</span>
		<a href="javascript:;" class="int"><input id="goPage" onkeyup="value=value.replace(/[^\d]/g,'')" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))" maxlength="4"/></a>
		<span>页</span>
		<input type="button" class="dib" onclick="javascript:gotoSelectPage($('#goPage').val());return false;">确定</input>
	</div>
