	...
	<div class="pays-cont">
		 ...
		 <div class="cont-row row-1">
		 	<div class="title">
		 		...
		 		<p class="sub">请您在提交订单后尽快支付即可参加一元抢礼活动</p>
		 	</div>
		 	<div class="info">
		 		<h5 class="d-h5">订单信息</h5>
		 		<p class="p1">${activeOrder.orderCode }   数量${activeOrder.count }只    共计：¥${activeOrder.totalPrice }</p>
		...
		<div class="cont-row row-2">
			<div class="title">
				支付金额：<span class="d-price">￥${activeOrder.totalPrice }</span>
			</div>
			<ul class="list">
				<li>
			...
			<div class="btn-pays">
				<input type="hidden" name="orderId" value="${activeOrder.orderId }"/>
...
<script>
	...
	function jump(){
		window.location.href=__base_path__+"pc/pv/myWinningCode";
	}
...
//		})
		...
		$('#btnSub').click(function(){
			...
			var href = '';
			if(way == 'alipay'){
				href=__base_path__+'pc/pv/activeOrder/alipayPay?orderId=' + $('[name=orderId]').val();
			} else if(way == 'unionpay')
			{
				href=__base_path__+'pc/pv/activeOrder/unionpayPay?orderId=' + $('[name=orderId]').val();
			}else if(way == 'unionpay2')
			{
				href=__base_path__+'pc/pv/activeOrder/unionpayPay2?orderId=' + $('[name=orderId]').val();
